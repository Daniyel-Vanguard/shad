...
use shad\Crypto\Crypto;
use shad\Config\Config;
use shad\Tools\{
    Message,
    SignIn
};
...
public function __construct(string|null $session = null, array $option = [])
{
    ...
    $application = Application::Shad; // فقط استفاده از شاد
    ...
    {
        $file_name = is_null($file_name) ? "shad-downloader-" . random_int(0, 100) : $file_name;
        if (!$file_inline) {
    ...
    public function sendFile($file_path, $object_guid, $reply = null, $captipn = '', callable|null $progress = null)
    {
        ...
        $json = [
            ...
            "file_inline" => [
                ...
                "file_id" => $up['file_id'],
                "type" => "File",
                "file_name" => "shadBot." . $up["mime"], //$up['file_name'],
                "size" => $up['file_size'],
                "mime" => $up['mime'],
    ...
    public function sendMuzic($file_path, $object_guid, $reply = null, $captipn = '', callable|null $progress = null)
    {
        ...
        $json = [
            ...
            "file_inline" => [
                ...
                "file_id" => $up['file_id'],
                "type" => "Music",
                "file_name" => "shadMusic." . $up["mime"], //$up['file_name'],
                "size" => $up['file_size'],
                "mime" => $up['mime'] ?? "mp3",
                "time" => $getTime,
                "music_performer" => "shadai music",
                "access_hash_rec" => $up['hash_code']
            ],
        ];
        if (!empty($captipn)) {
    ...
    public function sendGifByInfo(string $object_guid, string $file_id, int $dc_id, string $hash, string $caption = "", int $size = 0, int $time = 0, int $height = 0, int $width = 0, string $file_name = "shadBot", string $thumb = '')
    {
    ...
    public function createJoinLink(string $group_guid, string $title = "shadBOT", bool $request_needed = true, int $usage_limit = 0, int $time = 0)
    {
        $json = [
            "object_guid" => $group_guid,
            "title" => $title,
...
{
    $data = json_decode(
        self::Config_set(file_get_contents("$session.sr"), "shad/Daniyel-client"),
        true
    );
    ...
    private function Metadata($markdown)
    {
        ...
        while (preg_match($markdown_re, $markdown, $matches)) {
            ...
            if (mb_strpos($group, '```') === 0) {
                ...
                $url = str_replace(")", "", $matches[9]) ?? "https://vddaniyel.top/";
